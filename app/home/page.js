...
export default function HomePage() {
    ...
    try {
      if (paymentMethod === 'razorpay') {
        ...
        const options = {
          ...
          name: order.notes?.name || 'Real Estate',
  ...
  useEffect(() => {
    if (seoData) {
      // Update document title (normalize old branding to Real Estate)
      if (seoData.metaTitle) {
        const t = seoData.metaTitle
        const isOldBrand = /NOVA\s*KEYS|NovaKeys|SPIRELEAP|Spireleap|SpireLeap/i.test(t)
        document.title = isOldBrand ? 'Real Estate' : t
      }
